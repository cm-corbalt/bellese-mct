...
public class MeasureDataRequirementService {
    ...
    public Map<String, Map<String, List<IQueryParameterType>>> getSearchParamMapForPatient(String patientId) {
        ...
        patientSearchParamMap.forEach((key, value) -> {
            if (key.equals("Patient")) {
                value.put("_id", Collections.singletonList(new StringParam(patientId)));
            } else {
                try {
                    SearchParameterUtil.getOnlyPatientSearchParamForResourceType(fhirContext, key)
                            .ifPresent(patientParam -> value.put(
                                    patientParam.getName(), Collections.singletonList(new ReferenceParam(patientId))));
    ...
    public Map<String, Map<String, List<IQueryParameterType>>> getSearchParamsByType() {
        ...
        if (hasDataReqLibrary.isPresent()) {
            dataRequirements = ((Library) new VersionConvertor_40_50(new BaseAdvisor_40_50())
                            .convertResource(hasDataReqLibrary.get()))
                    .getDataRequirement();
        } else {
        ...
        for (DataRequirement dataRequirement : dataRequirements) {
            ...
            if (dataRequirement.hasProfile()) {
                ...
                dataRequirement.getProfile().forEach(profile -> {
            ...
            if (dataRequirement.hasCodeFilter()) {
                ...
                dataRequirement.getCodeFilter().forEach(codeFilter -> {
                    valuesetInfoMap.get(fhirType).computeIfAbsent(codeFilter.getPath(), x -> new ArrayList<>());
                    valuesetInfoMap
                            .get(fhirType)
                            .get(codeFilter.getPath())
                            .add(new ValueSetInfo().withId(codeFilter.getValueSet()));
                });
            }
        }
        return result;
