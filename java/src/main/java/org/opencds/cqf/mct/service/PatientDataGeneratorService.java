...
public class PatientDataGeneratorService {
    ...
    public Bundle generatePatientData(Integer numTestCases) throws IOException, NoSuchMethodException {
        ...
        File cqlFile = new File(Objects.requireNonNull(
                Objects.requireNonNull(ClasspathUtil.class
                                .getClassLoader()
                                .getResource("configuration/patient-data-gen-libraries/" + "CMS104TestDataGenerator2026"
                                        + ".cql"))
                        .getFile(),
                "UTF-8"));
        ...
        LibrarySourceProvider librarySourceProvider =
                new DefaultLibrarySourceProvider(new kotlinx.io.files.Path(libraryDirectory));
        ...
        Environment environment =
                new Environment(libraryManager, new HashMap<>(Map.of("http://hl7.org/fhir", dataProvider)));
        if (numTestCases != null) {
            ...
            evaluationParamsBuilder.library(
                    versionedIdentifier,
                    new EvaluationParams.LibraryParams(new ArrayList<>(
                            Collections.singletonList(new EvaluationExpressionRef("TestDataGenerationResult")))));
    ...
    private Object getEvaluationResultValue(EvaluationResults results, VersionedIdentifier versionedIdentifier) {
        ...
        if (evaluationResult == null) {
            throw new IllegalStateException(
                    "CQL evaluation did not return results for " + formatIdentifier(versionedIdentifier)
                            + ". Available result libraries: "
    ...
    private void appendGeneratedPatientGroup(Bundle bundle, int numTestCases, String measureId) {
        ...
        String groupIdentifier = measureId + "-" + Integer.toString(numTestCases) + "N" + GENERATED_PATIENT_BASE;
        ...
        groupEntry.getRequest().setMethod(Bundle.HTTPVerb.PUT).setUrl("Group/" + groupIdentifier);
    }
