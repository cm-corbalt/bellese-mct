...
public class PatientDataService {
    ...
    public PatientDataService(Group patients, List<String> facilities) {
        ...
        patientIds = patients.getMember().stream()
                .map(x -> x.getEntity().getReference().replace("Patient/", ""))
    ...
    public List<GatherService.PatientBundle> resolvePatientBundles(MeasureEvaluationService measureEvaluationService) {
        ...
        facilityDataServices.forEach(facilityDataService -> {
            for (String patientId : facilityDataService.getFacilityPatientsFromGroup(patientIds)) {
                ...
                facilityDataService.getPatientData(
                        measureEvaluationService.getMeasureDataRequirementsService(), patientBundle);
                validationService.validate(
                        patientBundle,
                        facilityDataService.getFacilityId(),
                        measureEvaluationService
                                .getMeasureDataRequirementsService()
                                .getProfileMap());
