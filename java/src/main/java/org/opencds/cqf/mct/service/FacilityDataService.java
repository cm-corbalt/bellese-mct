...
public class FacilityDataService {
    ...
    public FacilityDataService(String facilityId) {
        ...
        facilityClient = SpringContext.getBean(FhirContext.class)
    ...
    public List<String> getFacilityPatientsFromGroup(List<String> patientIds) {
        return getAllFacilityPatients().getEntry().stream()
                .filter(x -> patientIds.contains(x.getResource().getIdElement().getIdPart()))
                .map(x -> x.getResource().getIdElement().getIdPart())
    ...
    public void getPatientData(
            MeasureDataRequirementService measureDataRequirementService, GatherService.PatientBundle patientBundle) {
        measureDataRequirementService
                .getSearchParamMapForPatient(patientBundle.getPatientId())
                .forEach((key, value) -> {
                    Bundle searchResult = (Bundle) facilityClient
                            .search()
                            .forResource(key)
                            .where(value)
                            .execute();
                    if (searchResult.hasEntry()) {
                        ...
                        patientBundle.addMissingPatientData(
                                key,
                                measureDataRequirementService
                                        .getValuesetInfoMap()
                                        .get(key));
