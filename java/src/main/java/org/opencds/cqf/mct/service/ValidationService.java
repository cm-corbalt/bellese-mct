...
public class ValidationService {
    ...
    public void validate(
            GatherService.PatientBundle patientBundle, String facilityId, Map<String, List<String>> profileMap) {
        patientBundle.getPatientData().getEntry().forEach(entry -> {
            if (entry.hasResource()) {
                ValidationOptions options = new ValidationOptions();
                DomainResource resource = (DomainResource) entry.getResource();
                resource.addExtension(new Extension()
                        .setUrl(MctConstants.LOCATION_EXTENSION_URL)
                        .setValue(new Reference(facilityId)));
                resource.getMeta()
                        .addTag()
                        .setSystem(MctConstants.LOCATION_TAG_SYSTEM)
                        .setDisplay(facilityId);
